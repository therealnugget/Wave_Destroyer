...
Potion::Potion(FVector2 position, PotionType type): currentHomeForce(start_in_speed), potionType(type), Behaviour(SubRBData(std::get<0>(potionTypeToName.at(type)), {std::get<1>(potionTypeToName.at(type))}, static_cast<FVector2>(potion_collider_size)* Physics::DefaultSquareVerticesVec, position, potion_size, std::initializer_list<FVector2>(), FVector2::Zero, -potion_size * .5f, Main::Tag::_default, true, nullptr, std::unordered_map<std::string, std::variant<FVector2, FVector2*>>(), std::unordered_map<std::string, bool>(), FVector2::Zero, .0f, 1.f, false, true, Main::empty_cc_init, true, .0f, Main::Layer::all)), destroyTimer(true, destroy_time), collected(false) {
	rb->SetCollisionCallback([this](Collision* col) {OnCollision(col); });
	SetUpdateNode(Main::Updates += [this]() {Update();});
}